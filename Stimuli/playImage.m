...
% 03/06/26      hn: when image size exceeds dstRect, force it to full
%               screen (hack- revisit)
...
[pixelX0 pixelY0] = deg2pixelxy(sv.x0,sv.y0,ex.setup);
...
[left,~]    = deg2pixelxy(sv.x0 - imW_DVA/2, sv.y0, ex.setup);
[right,~]   = deg2pixelxy(sv.x0 + imW_DVA/2, sv.y0, ex.setup);
[~,bottom]  = deg2pixelxy(sv.x0, sv.y0 - imH_DVA/2, ex.setup);
[~,top]     = deg2pixelxy(sv.x0, sv.y0 + imH_DVA/2, ex.setup);

% LC edit because of error in function DrawTexture: Invalid Rect Argument
% impossible rect values given
imW_pix = imW_DVA * sv.ppd;
imH_pix = imH_DVA * sv.ppd;

baseRect = [0 0 imW_pix imH_pix];
% dstRect     = [left, bottom, right, top];
dstRect = CenterRectOnPointd(baseRect, pixelX0, pixelY0);
