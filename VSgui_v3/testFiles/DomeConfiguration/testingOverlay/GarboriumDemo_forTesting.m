...
disp('use datapixx')
%PsychImaging('AddTask', 'General', 'EnableDataPixxM16OutputWithOverlay'); % disrupts this demo
...
% below I tested different blendig factors after enabling DatapixxM16 with
% overlay and opening the window with 0 to 1 colorrange (ie.omit the argument 128)
% [win winRect] = PsychImaging('OpenWindow', screenid);
% 
Screen('BlendFunction', win, GL_SRC_ALPHA, GL_ONE);  % original values: shows mostly white but texture boundaries are not visible
%Screen('BlendFunction', win, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_COLOR); % shows mostly white but texture boundaries are not visible
%Screen('BlendFunction', win, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); % texture 
% %boundaries visible background is dark gray; But each Gabor by itselfs looks fine
%Screen('BlendFunction', win, GL_SRC_ALPHA, GL_ONE_MINUS_DST_ALPHA); % texture backgrounds are visible and very dark
%Screen('BlendFunction', win, GL_SRC_ALPHA, GL_ONE_MINUS_DST_COLOR); % blend factor only valid for the source
%Screen('BlendFunction', win, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA); % no Gabors visible on mid-gray textures
%Screen('BlendFunction', win, GL_SRC_COLOR, GL_ONE_MINUS_SRC_ALPHA); % The blending factor supplied for the source is only valid only for the destination
%Screen('BlendFunction', win, GL_DST_ALPHA, GL_ONE); % texure edges visible when gabors cross; overall brighter and gabors partly visible
%Screen('BlendFunction', win, GL_SRC_COLOR, GL_ONE_MINUS_SRC_ALPHA); % The blending factor supplied for the source is only valid only for the destination
