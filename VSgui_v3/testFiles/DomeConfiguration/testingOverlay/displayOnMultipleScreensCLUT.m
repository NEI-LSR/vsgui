function displayOnMulipleScreens(ngabors)
% based off GarboriumDemo([ngabors=200] [, internalRotation=0]) -- 
%
...
% 04/19/2024 modified to test display on multiple screens
...
screenid1 = max(Screen('Screens'));
screenid2 = max(Screen('Screens'))-1;
...
[win1 winRect1] = PsychImaging('OpenWindow', screenid1, 128);
[win2 winRect2] = PsychImaging('OpenWindow', screenid2, 128);
...
[ex.setup.window ex.setup.screenRect] = PsychImaging('OpenWindow',screenid1);
...
Screen('BlendFunction', win1, GL_SRC_ALPHA, GL_ONE);
Screen('BlendFunction', win2, GL_SRC_ALPHA, GL_ONE);
...
gabortex1=Screen('MakeTexture', win1, m, [], [], 2);
gabortex2=Screen('MakeTexture', win2, m, [], [], 2);
...
inrect = repmat(texrect1', 1, ngabors);

dstRects = zeros(4, ngabors);
for i=1:ngabors
    scale(i) = 2*(0.1 + 0.9 * randn);
    dstRects(:, i) = CenterRectOnPoint(texrect1 * scale(i), rand * w, rand * h)';
end
...
while ~KbCheck
    ...
    Screen('DrawTextures', win1, gabortex1, srcRect, dstRects, rotAngles, [], 0.5, [], [], sflags);
     Screen('DrawTextures', win2, gabortex2, srcRect, dstRects, rotAngles, [], 0.5, [], [], sflags);
