    ...
    switch ex.setup.ephys
        case 'sglx'
            ...
            if IsSaving(hSGL)
                ...
                if length(ex.Header.fileNameTrellis)>60
                    fNT = GetDataDir(hSGL);
                else
                    fNT = ex.Header.fileNameTrellis;
                end
                set(handles.fileNameTrellis,'String',fNT)
            end
        case 'gv'
            ...
            if strcmpi(status.status,'recording')
                ...
                if length(ex.Header.fileNameTrellis)>60
                    fNT = strrep(ex.Header.fileNameTrellis,'C:\Users\nienborg_group','~');
                else fNT = ex.Header.fileNameTrellis;
                end
                set(handles.fileNameTrellis,'String',fNT)
            end
    end
