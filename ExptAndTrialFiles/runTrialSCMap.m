...
% 01/07/26  hn: included ITI and fUS pulses
% 02/10/26  hn/bt: modified ITI implementation
...
while stayInLoop && ex.quit == 0
    ...
    % fUS TRIGGERS ------------------------------- new addition on 01/07/26
    if ~isempty(fUS_on)
        if state== ex.states.FPHOLD && ttime > time_startFixation + fUS_triggerOnsetTime &&~ fUS_triggered           
