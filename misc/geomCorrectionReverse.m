function [outX,outY] = geomCorrection(x,y,exSetup,invFlag)
% Geometry correction
...
if ~invFlag
    ...
    % Find index of desired position within native screen pixels grid
    [~,xInd]    = min(abs(exSetup.scal.XGRID_WARP(1,:)-x),[],2);
    [~,yInd]    = min(abs(exSetup.scal.YGRID_WARP(:,1)'-y),[],2);
    
    % Take corresponding pixel values of the indices
    pixelXTmp  = exSetup.scal.XGRID_PREWARP(yInd,xInd)';
    pixelYTmp  = exSetup.scal.YGRID_PREWARP(yInd,xInd);
