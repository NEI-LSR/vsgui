...
% 03/21/26  hn: added inverseWarpMap as field of ex.setup.scal to keep
%           track
...
switch lower(type)
    case 'setup' 
        ...
        ex.setup.vertEyeSign = -1; % flip sign of vertical eye position, need to investigate why it's necessary
       ...
       % monocular signals; for Propixx 1e7 works
        ...
        ex.setup.warpingCalibrationFile = fullfile(ex.setup.VSdirRoot ,...
            ...
            'calibfile_harish_DVA_updated260305.mat');
            % 'calibfile_harish_DVA_updated260305.mat');
if ~exist(ex.setup.warpingCalibrationFile,"file")
    ...
    case 'display'
        ...
        ex.setup.scal.inverseWarpMap = ptb_geometry_inverseWarpMap;
    
        % Change position of synch pulse are for the dome
        % ex.synch.Pos = [1446; 236]; % Busse procedure hand mapped warp
        % ex.synch.Pos = [470; 90]; % Harish automatic warping map
        ex.synch.Pos = [470; 90]; % new photodiode
        ex.synch.PSz = 70;  
