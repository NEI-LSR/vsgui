...
% Textcolor:
Screen('TextColor', scal.windowPtr, [255 0 255]);
...
colorGrey       = [50   50  50   255];
colorWhite      = [255  255 255 255];
colorCyan       = [0    255 255 255];

% color gradient for dots per row, to be able to distinguish rows
% without alpha value! 
% This only works with radial grid so far!
colors = uint8(round(hsv(nDots)*255))';
% circle
colvect = colors;
...
% Draw our own cursor during dot selection colvectmode
...
while (adjustMode ~= 0)
    ...
    while (adjustMode == 1)
        ...
        for view = 0:sign(scal.stereoMode)
            ...
            % Fill to black
            Screen('FillRect', scal.windowPtr, colorGrey);
            ...
            if xGlobalCalibDots ~= dots
                ...
                bvlDrawCrosshair(scal.windowPtr, xmax, ymax, selectedDotDiam * 2, colorWhite);
            else
            ...
            Screen('DrawDots', scal.windowPtr, transpose(dots), dotDiamArray, colvect, [0 0], dotStyle);
