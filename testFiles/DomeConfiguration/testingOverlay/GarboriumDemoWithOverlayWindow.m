...
% into a random direction. Smetimes the gabors intersect, in that case
% you'll see a linear superposition of them.
%
% Each frame of the andimation contains 'ngabors' patches, ngabors defaults
...
% 04/23/2024   hn: integrated with DatapixxM16 mode to test overlay (it
% works on optoma projector together with datapixx2)
...
PsychImaging('AddTask', 'General', 'FloatingPoint32Bit','disableDithering',1); % added from pds
...
fprintf('%s %d %\n','blackindex',BlackIndex(win))
fprintf('%s %d %\n','whiteindex',WhiteIndex(win))
Screen('FillRect',win,GrayIndex(win,.5));  % background to start 
cnt = 0; 
% Animation loop: Run until any keypress:
while ~KbCheck
    %Screen('FillRect',win,GrayIndex(win,.5));  % background to start 
    Screen('FillRect',win,0);  % background to start 
    ...
    % step two: draw on overlay window
    % 
    DrawFormattedText(overlay,'Test Display','center',300,243);
    Screen('Drawdots',win,[100 200; 100 400],8,-1);  % 


    % step three: test luminance values for other ptb functions
    % A) lines
    linesx = reshape(ones(2,1)*[round(winRect(3)/2):10:round(winRect(3)/2)+200],1,42);
    linesy = repmat([round(winRect(4)/2),100],1,21);
    linesXY = [linesx;linesy];

    %cols = [-1:.1:1]; 
    % drawlines: colors go from -1 to 1
    cols = reshape(ones(6,1)*[-1:.1:1],3,42);
    Screen('DrawLines', win, linesXY, 5,cols);
    
    % B) rect
    % fillrect: colors go from -1 to 1 but need to be given as 3-by-n
    % column matrix (same scalar color for each RGB row works)
    rect = repmat([0 0 20 20]',1,21) + ones(4,1)*[0:30:600];
    cols = reshape(ones(3,1)*[-1:.1:1],3,21);
    Screen('FillRect',win,cols,rect);


    % % works for one scalar color and rect value at a time    
    % % looping through colors of one rect at a time
    % if cnt>=21
    %     cnt=0;q
    % end
    % cnt = cnt+1;
    % rect1 = round(winRect/4);
    % rect2 = [0 0 30 30; 40 0 70 30 ]';
    % cols = [-1:.1:1];
    % %Screen('FillRect',win,cols(cnt),rect1);
    % %Screen('FillRect',win,cols(cnt),rect2);
    % %DrawFormattedText(overlay,sprintf('%s %1.1g','color index:',cols(cnt)),100,300,243);
    
    % C) dots
    dx = [100:10:300];
    dy = ones(1,21)*600;
    Screen('Drawdots',win,[dx;dy],8,cols);  % 
