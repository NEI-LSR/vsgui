...
switch ex.setup.ephys
    case 'sglx'
        ...
        [mat, ~] = Fetch(hSGL, 0, 0, begin_idx, end_idx-begin_idx, ...
            sum(nChannels)-2:sum(nChannels)-1);
        word_idx = find(any(diff(mat)~= 0, 2))+1;
        % initialize
        dig = nan(1,length(word_idx));
        tstamps = nan(1,length(word_idx));
        for i = 1:length(word_idx)
            w1 = (dec2bin(mat(word_idx(i), 1), 16)=='1')|...
                (circshift(dec2bin(mat(word_idx(i), 2), 16)=='1', -1));
