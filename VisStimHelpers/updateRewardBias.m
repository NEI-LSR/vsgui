function [ex,switchTrials] = updateRewardBias(ex,switchTrials,dailyLog)

% function [ex,switchTrials] = updateRewardBias(ex,switchTrials,dailyLog)
% 
% helper function to update the rewardBias. This was previously included 
% as part of runEpxt.m
%
% history
% 01/29/26  hn: moved from runExpt for code readability (no code changes
%           otherwise)
...
if ex.exp.afc && strcmp(ex.reward.type((end-8):end),'Asymmetry')
    ...
    switchTrials = ex.reward.switchTrials(ex.reward.switchTrials >= ...
        (dailyLog.nTrialsPerDay));
end
