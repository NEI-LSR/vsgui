...
version = '2.0.7'; % 08/15/25   HN: increased legibity of getDefaultSettings.m
                                ...
                                %   recordings
version = '3.0.0'; % 01/19/26   HN: merged version across rigs after 'hackathon'
                   % 02/10/26   hn: packaged +default, +ephys, +eylink
                   % 03/06/26   hn: makeImage (crop larger images to screen
                   %            size);  playImage (set dstRect to screen
                   %            size)
                   %            cmz: calibrateEye_dome
                   %                 checkEye (flip y-eye position, hard-coded for dome)
                   %            
                   % 03/13/26   hn: include plotRaster, re-order channels
                   %            accordint to sglx channel map for plotting
                   %            +ephys.sortChannelMap (new)
                   %            plotGuiHelpers/plotRaster (new)
                   %            plotGui.fig (modified)
                   %            plotGuiHelpers/plotFiles (modified)
                   %            plotGuiHelpers/plotPopulationTC(modified)
                   %03/21/26    hn: inverseWarpMap is now stored in
                   %            ex.setup.scal (in rigSEtups/nimhB1A19Dome)
