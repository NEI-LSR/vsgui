function ex = calibrateEye_dome(ex)
...
% iknew - 9.26.2022
% 11/03/2025    cmz: updated for use in the dome
...
for n = rp
    ...
    % if any([isinf(ex.eyeCal.LYGain) isinf(ex.eyeCal.LXGain) isinf(ex.eyeCal.RYGain) isinf(ex.eyeCal.RXGain)])
    %     % cmz 10/31/25: The problem this was intended to catch seems to have been
    %     % caused by a hardware issue with a bad BNC cable not transmitting
    %     % the LY signal, not to any problems with eyelink tracking the eye
    %     % cmz 02/26/26: hardware issue is intermittently coming back
    %     keyboard
    % end
...
% iknew
% unit - pixel/adc
% cmz: this is superfluous right? Already done in the loop.
