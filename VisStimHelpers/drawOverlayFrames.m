...
if isempty(goodTPos) % we only have the fixation window and RF
    ...
    if geomCorrect
        ...
        winDeg = [-ex.targ.WinW -ex.targ.WinH ex.targ.WinW ex.targ.WinH] + ...
            [ex.targ.PosDeg(1,:) ex.targ.PosDeg(1,:)];
        [winPix(1),~] = deg2pixelxy(winDeg(1),ex.targ.PosDeg(1,2),ex.setup);
        [~,winPix(2)] = deg2pixelxy(ex.targ.PosDeg(1,1),winDeg(2),ex.setup);
        [winPix(3),~] = deg2pixelxy(winDeg(3),ex.targ.PosDeg(1,2),ex.setup);
        [~,winPix(4)] = deg2pixelxy(ex.targ.PosDeg(1,1),winDeg(4),ex.setup);
